...
public class QS_Optimization3{
        ...
        //uses Insertion Sort algorithm between specific indecies
                ...
                while (boundary <= r){
                        ...
                        if (j > l && a[j-1].compareTo(a[j]) <= 0){ //corrects for possible undercounting of comparisons
                                comparisons+=1;
                        }
                        a[j] = temp;
                        boundary+=1;
        ...
        public static void partition(String a[], int l, int h, int s){
                if (h - l > s){
                String temp = "";
                ...
                partition(a, l, m-1, s); //partitions lower half (from index l to the exchanged )
                partition(a, m+1, h, s); //partitions upper half
            	}
                else{
                    insertionSort(a, l, h);
                }
        }
        public static void sort (String a[], int s){ //partitions the array into two halves, then "sorts" each one individually
                partition(a, 0, a.length-1, s);
        }
